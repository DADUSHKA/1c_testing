...
// Контролирует, информирует и помогает заполнить табличную часть активными договорами.
//
// Параметры:
//  ТекущийОбъект  - ДанныеФормыСтруктура - объект редактирования.
//  ЭлементыФормы  - ВсеЭлементыФормы - элементы формы текущего объекта.
//  ТекущаяТаблица  - ТаблицаФормы - табличная часть текущего объекта.
//
Асинх Процедура КонтрольЗаполненияТаблицыАктивнымиДоговорамиАсинх(ТекущийОбъект, ЭлементыФормы, ТекущаяТаблица) Экспорт
	
	ПараметрыФормы = ОбщегоНазначенияВызовСервера.ПараметрыФормыВыбораПодобныхАктивныхДоговоров(ТекущаяТаблица.ТекущиеДанные.Договор);
	
	Если ПараметрыФормы.АктивностьТекущегоДоговора Тогда
		Возврат;
	Иначе	
		
		Если Не ПараметрыФормы.ЕстьАльтернатива Тогда
			
			Информация = "Выбранный договор не активен.
			|Запись в таблицу не возможна.";
			
			ПоказатьПредупреждение(, Информация,, "Предупреждение");
			
		КонецЕсли;	
		
		ТекущийОбъект.Договоры.Удалить(ТекущаяТаблица.ТекущиеДанные.НомерСтроки - 1);
		
	КонецЕсли;
	
	Если ПараметрыФормы.ЕстьАльтернатива Тогда
				
		Информация = НСтр("ru = 'Выбранный договор не активен.
		|Запись в таблицу не возможна.
		|Есть подобные активные договоры.
		|Открыть форму выбора?'");
		
		Ответ = Ждать ВопросАсинх(Информация, РежимДиалогаВопрос.ДаНет);
		
		Если Ответ = КодВозвратаДиалога.Да Тогда
			ОткрытьФорму("Справочник.Договоры.Форма.ФормаВыбора", ПараметрыФормы, ЭлементыФормы.Договора);
		КонецЕсли;
