...
//
// Параметры:
//  ЭлементФормы  - ТаблицаФормы(ПолеФормы) - элемент формы текущего объекта родитель открываемой формы выбора.
//  Ссылка  - СправочникСсылка.Договоры - ссылка на выбранный договор.
//
Асинх Процедура КонтрольЗаполненияТаблицыАктивнымиДоговорамиАсинх(ЭлементФормы, Ссылка) Экспорт
	
	ПараметрыФормы = ОбщегоНазначенияВызовСервера.ПараметрыФормыВыбораПодобныхАктивныхДоговоров(Ссылка);
	...
	Иначе	
		...
		Если ПараметрыФормы.ЕстьАльтернатива Тогда
			...
			Если Ответ = КодВозвратаДиалога.Да Тогда
				ОткрытьФорму("Справочник.Договоры.Форма.ФормаВыбора", ПараметрыФормы, ЭлементФормы);
			КонецЕсли;
			
		Иначе
						
			ПоказатьПредупреждение(, "Выбранный договор не активен.", , "Предупреждение");
