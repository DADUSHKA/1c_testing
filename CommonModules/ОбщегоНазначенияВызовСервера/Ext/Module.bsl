...
// Формирует параметры для открытия формы выбора похожих активных договоров и информационной логики.
//
// Параметры:
//  СсылкаНаДоговор  - СправочникСсылка.Договоры - ссылка на выбранный договор.
//
Функция ПараметрыФормыВыбораПодобныхАктивныхДоговоров(СсылкаНаДоговор) Экспорт
	
	ПараметрыОтбора = Новый Структура;
	ДоступныеДоговора = Новый Массив;
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("АктивностьТекущегоДоговора", Истина);
	ПараметрыФормы.Вставить("ЕстьАльтернатива", Ложь);
	...
	|ИЗ
	|	Справочник.Договоры КАК Договоры
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ВыбранныйДоговор КАК ВТ_ВыбранныйДоговор
	|		ПО Договоры.Наименование = ВТ_ВыбранныйДоговор.Наименование
	|ГДЕ
	|	ВТ_ВыбранныйДоговор.Ссылка <> Договоры.Ссылка
	|	И Договоры.Активен";
	
	Запрос.УстановитьПараметр("Ссылка", СсылкаНаДоговор);
	ПолученныеДанные = Запрос.ВыполнитьПакетСПромежуточнымиДанными();
	
	Если ПолученныеДанные[0].Выгрузить()[0].Активен Тогда
		
		Возврат ПараметрыФормы;
		
	КонецЕсли;
	
	АльтернативныеДоговора = ПолученныеДанные[1].Выбрать();
	ПараметрыФормы.АктивностьТекущегоДоговора = Ложь;
	
	Если АльтернативныеДоговора.Количество() > 0 Тогда 
		
		Пока АльтернативныеДоговора.Следующий() Цикл
			ДоступныеДоговора.Добавить(АльтернативныеДоговора.Ссылка); 
		КонецЦикла;
		
		ПараметрыОтбора.Вставить("Ссылка", ДоступныеДоговора);	
		ПараметрыФормы.Вставить("Отбор", ПараметрыОтбора);
		ПараметрыФормы.ЕстьАльтернатива = Истина;
