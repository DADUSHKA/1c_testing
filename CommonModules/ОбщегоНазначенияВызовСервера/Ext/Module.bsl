...
// Формирует параметры для открытия формы выбора похожих активных договоров.
//
// Параметры:
//  СсылкаНаДоговор  - СправочникСсылка.Договоры - ссылка на выбранный договор для табличной части объекта.
//
Функция ПараметрыФормыВыбораПодобныхАктивныхДоговоров(СсылкаНаДоговор) Экспорт
	
	ПараметрыОтбора = Новый Структура;
	ВсеДоговора = Новый Массив;
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("АктивностьТекущегоДоговора", Ложь);
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	Договоры.Активен КАК Статус,
	|	Договоры.Ссылка КАК Ссылка
	|ИЗ
	|	ВТ_ВыбранныйДоговор КАК ВТ_ВыбранныйДоговор,
	|	Справочник.Договоры КАК Договоры
	|ГДЕ
	|	Договоры.Наименование = ВТ_ВыбранныйДоговор.Наименование
	|	И ВТ_ВыбранныйДоговор.Ссылка <> Договоры.Ссылка
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ВТ_ВыбранныйДоговор.Активен,
	|	ВТ_ВыбранныйДоговор.Ссылка
	|ИЗ
	|	ВТ_ВыбранныйДоговор КАК ВТ_ВыбранныйДоговор";
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Если ВыборкаДетальныеЗаписи.Статус И ВыборкаДетальныеЗаписи.Ссылка = СсылкаНаДоговор Тогда
			
			ПараметрыФормы.АктивностьТекущегоДоговора = ВыборкаДетальныеЗаписи.Статус;
		...
		Если ВыборкаДетальныеЗаписи.Статус И ВыборкаДетальныеЗаписи.Ссылка <> СсылкаНаДоговор Тогда
			...
			ВсеДоговора.Добавить(ВыборкаДетальныеЗаписи.Ссылка); 
			
		КонецЕсли;
		
	КонецЦикла;
	
	Если Не ПараметрыФормы.АктивностьТекущегоДоговора И ВсеДоговора.Количество() > 0 Тогда
		
		ПараметрыОтбора.Вставить("Ссылка", ВсеДоговора);	
